...
from .TaskBase import (
    BaseTask, Level, CONFIG, create_ssh_connection, ssh_exec,
    grade_percent, require_keys, decrypt_password
)
...
class MirrorFortiGateTask(BaseTask):
    ...
    def _get_hostname(SSH: paramiko.SSHClient) -> str:
        ...
        """
        EXIT_CODE, STDOUT_TEXT, _ = ssh_exec(
            SSH, "get system status | grep Hostname", label="get hostname"
        )
    ...
    def _check_performance_metric(
        self, HOSTNAME: str, HOST: str, METRIC_NAME: str,
        ...
        LEVEL = self._grade(VALUE, WARN_THRESHOLD, CRIT_THRESHOLD)
        if LEVEL == Level.ERROR:
            self.add_result(LEVEL, f"站点{SITE_NAME}镜像飞塔防火墙{METRIC_NAME} 使用率解析失败")
        else:
            msg = (
                f"站点{SITE_NAME}镜像飞塔防火墙{METRIC_NAME} {VALUE}%"
                f"（预警WARN:{WARN_THRESHOLD}/严重CRITICAL:{CRIT_THRESHOLD}%）"
            )
            self.add_result(LEVEL, msg)
    ...
    def run_single(self, HOST: str) -> None:
        ...
        try:
            ...
            HOSTNAME = self._get_hostname(SSH)

            # 1) 日志盘
            _, DISK_OUTPUT, _ = ssh_exec(SSH, "diagnose sys logdisk usage", label="logdisk usage")
            DISK_PERCENT = self._parse_disk_percent(DISK_OUTPUT)
            # 从主机名中提取站点名（如HX00-Sniffer-FortiGate-1801F -> HX00）
            SITE_NAME = HOSTNAME.split('-')[0] if '-' in HOSTNAME else HOSTNAME
            if DISK_PERCENT is None:
                ...
                msg = (
                    f"站点{SITE_NAME}镜像飞塔防火墙日志盘 {DISK_PERCENT}%"
                    f"（预警WARN:{self.DISK_WARN}/严重CRITICAL:{self.DISK_CRIT}%）"
                )
                self.add_result(Level.CRIT, msg)
            ...
            CPU_USED, MEM_USED, UPTIME_DAYS = self._parse_perf_status(PERF_OUTPUT)

            # 调试：打印 perf 回显与初次解析结果（仅目标主机）
            # 兜底：若 Uptime 仍为空，再从 get system status 抓一次（同样考虑分页）
            if UPTIME_DAYS is None:
                # 尽量直接读完整的 system status（不要带 grep，部分设备不支持管道/同样会分页）
                SYS_STATUS_FULL = self._ssh_exec_paged(SSH, "get system status")
                _, _, UPTIME_TRY = self._parse_perf_status(SYS_STATUS_FULL)
            ...
            if UPTIME_DAYS is None:
                ...
                self.add_result(
                    Level.CRIT,
                    f"站点{SITE_NAME}镜像飞塔防火墙启动时间 仅 {UPTIME_DAYS:.2f} 天"
                    f"（< {self.MIN_UPTIME_DAYS} 天）"
                )
            else:
