...
# - no_connection状态：直接标记为ERROR，跳过备份请求，不需要检查URL
# - success状态：尝试备份配置，但需检查URL响应内容是否为"node not found"
# - never/failing状态：直接标记为ERROR，跳过备份请求
# - 其他状态：标记为ERROR，跳过备份请求
# - 响应内容检查：检测"node not found"响应，标记为ERROR
# - 节点异常：节点连接失败时标记为WARN
#
# 架构重构优化:
...
# - 日志压缩：自动将非当日的.log文件压缩为{日期}-OxidizedTaskBackup.zip
#
# 配置验证:
...
class OxidizedTask(BaseTask):
    ...
    def __init__(self):
        ...
        BASE_LOG_DIRECTORY_PATH = CONFIG["settings"]["log_dir"]
        # V10新结构：输出到 LOG/OxidizedTask/OxidizedTaskBackup/
        self.LOG_DIR = os.path.join(BASE_LOG_DIRECTORY_PATH, "OxidizedTask", "OxidizedTaskBackup")
        # 用于存储所有节点的设备信息
        self.ALL_DEVICES = []
        self.TOTAL_FAILED = 0
    ...
    def run_single(self, BASE_URL: str) -> None:
        ...
        try:
            ...
            HTTP_RESPONSE = SESSION.get(BASE_URL, timeout=(DEFAULT_HTTP_TIMEOUT, DEFAULT_HTTP_TIMEOUT))
        ...
        STATUS_ELEMENT_LIST = TREE.xpath("//*[@id='nodesTable']/tbody/tr/td[4]/div")


        # 如果第一种XPath没找到，尝试其他可能的XPath
        if not STATUS_ELEMENT_LIST:
            STATUS_ELEMENT_LIST = TREE.xpath("//table/tbody/tr/td[4]/div")
        if not STATUS_ELEMENT_LIST:
            STATUS_ELEMENT_LIST = TREE.xpath("//tr/td[4]/div")
        if not STATUS_ELEMENT_LIST:
            STATUS_ELEMENT_LIST = TREE.xpath("//td[4]/div")
        if not STATUS_ELEMENT_LIST:
            # 尝试查找所有包含class的div元素
            STATUS_ELEMENT_LIST = TREE.xpath(
        ...
        DEVICE_STATUSES = []
        for DIV in STATUS_ELEMENT_LIST:
            # 检查div的class属性，提取状态信息
            DIV_ELEMENT_CLASS = DIV.get('class', '')
            if DIV_ELEMENT_CLASS in ['success', 'no_connection', 'never', 'failing']:
                DEVICE_STATUSES.append(DIV_ELEMENT_CLASS)
            else:
        ...
        for DEVICE_NAME, GROUP_NAME, STATUS in zip(DEVICE_NAMES, GROUP_NAMES, DEVICE_STATUSES):
            ...
            if STATUS == 'success':
                # 尝试获取配置
                FETCH_UNIFORM_RESOURCE_LOCATOR = f"{BASE_URL.replace('/nodes', '')}/node/fetch/{GROUP}/{DEVICE}"
                try:
                    CONFIGURATION_HTTP_RESPONSE = SESSION.get(FETCH_UNIFORM_RESOURCE_LOCATOR, timeout=DEFAULT_HTTP_TIMEOUT)
                    CONFIGURATION_HTTP_RESPONSE.raise_for_status()


                    # 检查响应内容是否为"node not found"
                    if "node not found" in CONFIGURATION_HTTP_RESPONSE.text.lower():
                    ...
                    LOG_DOCUMENT_PATH = os.path.join(
                        self.LOG_DIR,
                        f"{TODAY_DATE_TEXT}-{safe_filename(DEVICE)}.log"
                    )
                    with open(LOG_DOCUMENT_PATH, "w", encoding="utf-8") as FILE_HANDLE:
                        FILE_HANDLE.write(CONFIGURATION_HTTP_RESPONSE.text)
                    LOCAL_BACKUP_COUNTER += 1
    ...
    def _pack_backups_excluding_today(self) -> None:
        ...
        for NAME in os.listdir(self.LOG_DIR):
            ...
            if DATE_TEXT == TODAY_DATE_TEXT:
                continue
            DATE_GROUPS.setdefault(DATE_TEXT, []).append(NAME)
        if not DATE_GROUPS:
            return
        # 逐日期打包
        for DATE_TEXT, FILES in sorted(DATE_GROUPS.items()):
            ZIP_DOCUMENT_NAME = f"{DATE_TEXT}-OxidizedTaskBackup.zip"
            ZIP_DOCUMENT_PATH = os.path.join(self.LOG_DIR, ZIP_DOCUMENT_NAME)
            if os.path.exists(ZIP_DOCUMENT_PATH):
                continue
            try:
                with zipfile.ZipFile(ZIP_DOCUMENT_PATH, 'w', zipfile.ZIP_DEFLATED) as ZIPF:
                    for F in FILES:
                        DOCUMENT_PATH = os.path.join(self.LOG_DIR, F)
                        if os.path.isfile(DOCUMENT_PATH):
                            ZIPF.write(DOCUMENT_PATH, arcname=F)
                # 打包成功后删除原文件
                for F in FILES:
                    DOCUMENT_PATH = os.path.join(self.LOG_DIR, F)
                    try:
