...
# 目标:检查NTP Chronyd和TACACS+ tac_plus服务状态
#
...
# 配置说明:复用ESLogstashTask配置，使用相同的SSH认证和服务器列表
# 特点:支持多服务器并行检查
...
class ServiceCheckTask(BaseTask):
    ...
    def run_single(self, hostname: str) -> None:
        ...
        Args:
            hostname: 主机名
        """
        IP_ADDRESS = self.HOSTS[hostname]


        try:
            # 检查Chronyd服务
            self._check_chronyd_service(hostname, IP_ADDRESS)


            # 检查TACACS+服务
            self._check_tacplus_service(hostname, IP_ADDRESS)


        except Exception as error:
            self.add_result(Level.ERROR, f"{hostname}({IP_ADDRESS}) 服务检查异常: {error!r}")


    # 检查Chronyd NTP服务状态：检查服务运行状态、NTP同步状态、进程和端口监听
    def _check_chronyd_service(self, hostname: str, IP_ADDRESS: str) -> None:
        """检查Chronyd NTP服务状态
        ...
        try:
            # 建立SSH连接
            SSH_CONNECTION = create_ssh_connection(IP_ADDRESS, 22, self.USERNAME, self.PASSWORD)


            # 检查chronyd服务状态
            STATUS_COMMAND = "systemctl status chronyd"
            EXIT_CODE, STATUS_OUTPUT, STDERR = ssh_exec(SSH_CONNECTION, STATUS_COMMAND)
            ...
            EXIT_CODE, TRACKING_OUTPUT, STDERR = ssh_exec(SSH_CONNECTION, TRACKING_COMMAND)
            ...
            # 检查chronyd进程
            PROCESS_STATUS_COMMAND = "ps -ef | grep chronyd"
            EXIT_CODE, PS_OUTPUT, STDERR = ssh_exec(SSH_CONNECTION, PROCESS_STATUS_COMMAND)
            ...
            # 检查UDP 123端口
            SOCKET_STATUS_COMMAND = "ss -ulpn | grep chronyd"
            EXIT_CODE, SS_OUTPUT, STDERR = ssh_exec(SSH_CONNECTION, SOCKET_STATUS_COMMAND)
    ...
    def _parse_chronyc_tracking(self, hostname: str, TRACKING_OUTPUT: str) -> None:
        ...
        try:
            # 检查Reference ID
            REFERENCE_IDENTIFIER_MATCH = re.search(
                r'Reference ID\s*:\s*([0-9A-F]+)\s*\(([^)]+)\)',
                TRACKING_OUTPUT
            )
            if REFERENCE_IDENTIFIER_MATCH:
                REFERENCE_IP_ADDRESS = REFERENCE_IDENTIFIER_MATCH.group(2)
                if REFERENCE_IP_ADDRESS != "0.0.0.0":
                    self.add_result(Level.OK, f"{hostname} Chronyd参考服务器: {REFERENCE_IP_ADDRESS}")
                else:
            ...
            OFFSET_MATCH = re.search(
                r'Last offset\s*:\s*([+-]?\d+\.?\d*)\s*seconds',
                TRACKING_OUTPUT
            )
            if OFFSET_MATCH:
    ...
    def _check_tacplus_service(self, hostname: str, IP_ADDRESS: str) -> None:
        ...
        try:
            ...
            # 检查tac_plus服务状态
            STATUS_COMMAND = "systemctl status tac_plus"
            EXIT_CODE, STATUS_OUTPUT, STDERR = ssh_exec(SSH_CONNECTION, STATUS_COMMAND)
            ...
            # 检查tac_plus进程
            PROCESS_STATUS_COMMAND = "ps -ef | grep tac_plus"
            EXIT_CODE, PS_OUTPUT, STDERR = ssh_exec(SSH_CONNECTION, PROCESS_STATUS_COMMAND)
            ...
            # 检查TCP 49端口
            SOCKET_STATUS_COMMAND = "ss -tulnp | grep 49"
            EXIT_CODE, SS_OUTPUT, STDERR = ssh_exec(SSH_CONNECTION, SOCKET_STATUS_COMMAND)
