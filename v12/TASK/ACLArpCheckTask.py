...
#
# 配置说明:
# - 输入文件:自动使用当天日期的DeviceBackupTask输出文件（硬编码路径：LOG/DeviceBackupTask/{日期}-关键设备配置备份输出EXCEL基础任务.xlsx）
# - ARP日志文件:CHECKRULE/ARP.log（硬编码路径）
# - 输出目录:LOG/ACLArpCheckTask/（硬编码路径，V10新结构：从ACL/ACLArpCheckTask迁移）
# - 脚本输出目录:LOG/ACLArpCheckTask/ConfigureOutput/（从settings.log_dir读取LOG目录，V10新结构：从CONFIGURATION/ACLArpCheckTask/日期迁移，每次任务自动清空）
...
# - 脚本输出:操作脚本和回退脚本，根据原始ACL定义是否包含extended关键字自动决定是否添加extended（支持CS-N9K不带extended和LINK-AS带extended的区分，V11优化）
# - 汇总输出控制:通过Config.yaml的ACLArpCheckTask.enable_summary_output开关控制汇总统计输出，默认关闭
# - 依赖库:xlsxwriter
...
from .CiscoBase import (
    parse_acl, parse_acl_full, find_acl_blocks_in_column, extract_acl_rules_from_column, is_acl_rule,
    ip_and_wildcard_to_network, host_to_network, cidr_to_network, any_to_network
)

# ACL解析已迁移到 CiscoBase.parse_acl_full，删除本地重复的正则表达式定义
# 网络转换函数（ip_and_wildcard_to_network, host_to_network, cidr_to_network, any_to_network）已从 CiscoBase 导入
...
# parse_acl_line 已迁移到 CiscoBase.parse_acl_full，删除本地重复实现
# 注意：使用 parse_acl_full 而不是 parse_acl_network_only，因为需要支持 any 规则的检测
# 解析ARP表，返回有MAC的IP集合（INCOMPLETE不计入）
# 解析ARP表，返回有MAC的IP集合（可按配置忽略特定段，如X.X.108.X）
def parse_arp_table(arp_path: str) -> Set[IPv4Address]:
    """解析ARP表文件，提取有MAC的IP地址

    Args:
        arp_path: ARP日志文件路径

    Returns:
        IP地址集合，包含所有有MAC的IP地址（排除INCOMPLETE）
    """
    ...
    with open(arp_path, "r", encoding="utf-8", errors="ignore") as file_handle:
        for ARP_LOG_LINE in file_handle:
            ...
            if mac_or_state.upper() == "INCOMPLETE":
                ...
                continue
            try:
                internet_protocol_address = IPv4Address(ip_str)
            except Exception:
                continue
            # 忽略 X.X.108.X 这类第三段等于指定值的 IP
            if ignore_octet_third_values:
                third = int(str(internet_protocol_address).split(".")[2])
                if third in ignore_octet_third_values:
                    continue
            arp_ok_ip_addresses.add(internet_protocol_address)
    return arp_ok_ip_addresses

# 解析NoUseIPRange.log文件，返回未使用IP网段列表
# 解析NoUseIPRange.log文件，返回未使用IP网段列表
def parse_no_use_ip_ranges(no_use_file: str) -> List[str]:
    """解析NoUseIPRange文件，提取不需要检查的IP范围列表

    Args:
        no_use_file: NoUseIPRange文件路径
...
def check_no_use_ip_range(network: IPv4Network, no_use_ranges: List[str]) -> bool:
    """检查IP网段是否命中NoUseIPRange列表

    Args:
        network: 要检查的IPv4Network对象
        no_use_ranges: NoUseIPRange列表

    Returns:
        如果命中则返回True，否则返回False
    """
    network_text = str(network.network_address)
    for no_use_range in no_use_ranges:
        # 检查网络地址是否以NoUseIPRange中的前缀开头（模糊匹配）
        if network_text.startswith(no_use_range):
            return True
    return False
...
def check_arp_match_optimized(network: IPv4Network, arp_ok_ip_addresses: Set[IPv4Address]) -> bool:
    ...
    Args:
        network: 要检查的IPv4Network对象
        arp_ok_ip_addresses: 有MAC的IP地址集合

    Returns:
        如果网络中有IP地址在ARP表中则返回True，否则返回False
    """
    for internet_protocol_address in arp_ok_ip_addresses:
        if internet_protocol_address in network:
...
def create_rich_text_from_unicode_marked(text: str, workbook, color_type: str = 'red'):
    """将Unicode标记的文本转换为富文本格式

    Args:
        text: 包含Unicode标记的文本
        workbook: xlsxwriter工作簿对象
        color_type: 颜色类型（'red' 或 'orange'）

    Returns:
        富文本格式列表，用于xlsxwriter的write_rich_string方法
    """
    if '·' not in text:
        normal_formatting = workbook.add_format({'color': 'black'})
        return [normal_formatting, text]


    # 创建格式
    if color_type == 'red':
        color_formatting = workbook.add_format({'color': 'red'})
    elif color_type == 'orange':
        color_formatting = workbook.add_format({'color': 'orange'})
    else:
        color_formatting = workbook.add_format({'color': 'black'})
    ...
    for i, part in enumerate(parts):
        if part:  # 跳过空字符串
            if i % 2 == 0:  # 偶数索引：普通文本
                result_list.append(normal_formatting)
                result_list.append(part)
            else:  # 奇数索引：关键字（红色）
                result_list.append(color_formatting)
                result_list.append(part)
...
def process_acl_block_with_unicode_marking(
    ...
    arp_ok_ip_addresses, no_use_ranges, acl_configuration, workbook,
    sheet_name, platform_network_map
):
    """处理ACL块并使用Unicode标记进行标色

    Args:
        inputWorksheet: 输入工作表对象
        worksheet: 输出工作表对象
        column_index: 列索引
        start_row: 起始行
        end_row: 结束行
        arp_ok_ip_addresses: 有MAC的IP地址集合
        no_use_ranges: NoUseIPRange列表
        acl_configuration: ACL配置信息
        workbook: xlsxwriter工作簿对象
        sheet_name: 工作表名称
        platform_network_map: 平台网段映射

    Returns:
        元组：(红色计数, 黄色计数, 总计数, 标色规则信息列表)
    """
    red_count = 0
    yellow_count = 0
    total_count = 0
    colored_rules_information = []


    # 创建格式
    red_formatting = workbook.add_format({'color': 'red'})
    orange_formatting = workbook.add_format({'color': 'orange'})
    normal_formatting = workbook.add_format({'color': 'black'})
    background_formatting = workbook.add_format({'bg_color': '#E6F3FF'})
    ...
    for ROW in range(start_row, end_row + 1):
        ...
        # 解析ACL规则（使用 parse_acl_full 以支持 any 规则）
        parsed_rule, err = parse_acl_full(cleaned_text)
        if not parsed_rule:
            continue
        source_network = parsed_rule.src
        destination_network = parsed_rule.dst
        ...
        source_is_any = str(source_network) == "0.0.0.0/0"
        destination_is_any = str(destination_network) == "0.0.0.0/0"


        # 检查是否包含需要忽略的网段（如X.X.108.X）
        ignore_octet_third_values = acl_configuration["ignore_third_octet"]
        source_should_ignore = False
        destination_should_ignore = False


        if ignore_octet_third_values:
            source_text = str(source_network.network_address)
            destination_text = str(destination_network.network_address)
            ...
            if '.' in source_text:
                ...
                if len(source_parts) >= 3:
                    source_third = int(source_parts[2])
                    if source_third in ignore_octet_third_values:
                        source_should_ignore = True


            # 检查目的网络是否包含需要忽略的第三段
            if '.' in destination_text:
                destination_parts = destination_text.split('.')
                if len(destination_parts) >= 3:
                    destination_third = int(destination_parts[2])
                    if destination_third in ignore_octet_third_values:
                        destination_should_ignore = True


        # 检查网络是否在平台网段内（从公共配置读取）
        platform_networks = platform_network_map.get(sheet_name, [])
        source_in_platform = False
        destination_in_platform = False


        if platform_networks:
            # 检查源网络是否在平台网段内（重叠或包含关系）
            for platform_network in platform_networks:
                if (source_network.overlaps(platform_network) or
                        source_network.subnet_of(platform_network)):
                    source_in_platform = True
                    break
            # 检查目的网络是否在平台网段内（重叠或包含关系）
            for platform_network in platform_networks:
                if (destination_network.overlaps(platform_network) or
                        destination_network.subnet_of(platform_network)):
                    destination_in_platform = True
                    break


        # 检测逻辑：只检测在平台网段内的网络，且忽略ignore_third_octet
        if source_should_ignore or destination_should_ignore:
            ...
            if source_no_use or destination_no_use:
                ...
                should_detect = False
        elif source_is_any and destination_is_any:
            # any to any 不检测
            should_detect = False
        elif source_is_any:
            # 源为any，只检测目的是否在平台网段内且命中ARP
            if destination_in_platform:
                should_detect = not destination_hit
            else:
                should_detect = False
        elif destination_is_any:
            # 目的为any，只检测源是否在平台网段内且命中ARP
            if source_in_platform:
                should_detect = not source_hit
            else:
                should_detect = False
        elif source_in_platform and destination_in_platform:
            # 源和目的都在平台网段内，检测ARP匹配
            should_detect = (not source_hit) or (not destination_hit)
        elif source_in_platform:
            # 只有源在平台网段内，只检测源是否命中ARP
            should_detect = not source_hit
        elif destination_in_platform:
        ...
        if should_detect:
            ...
            if source_no_use or destination_no_use:
                ...
                rich_text = create_rich_text_from_unicode_marked(
                    marked_text, workbook, 'orange'
                )
                if len(rich_text) >= 3:
                    worksheet.write_rich_string(
                        ROW-1, column_index-1, *rich_text, background_formatting
                    )
                else:
                    worksheet.write(ROW-1, column_index-1, original_text, background_formatting)
                yellow_count += 1


                # 记录标色规则信息
                colored_rules_information.append({
                    'row': ROW,
                    'column': column_index,
                ...
                keywords_to_mark = []
                if not source_hit and not source_is_any:
                    keywords_to_mark.append(str(source_network.network_address))
                if not destination_hit and not destination_is_any:
                    keywords_to_mark.append(str(destination_network.network_address))
    ...
    return red_count, yellow_count, total_count, colored_rules_information
...
class ACLArpCheckTask(BaseTask):
    ...
    检查ACL规则在ARP表中的命中情况，使用xlsxwriter输出原位富文本标色，
    并基于标色结果按设备/ACL分组生成操作脚本与回退脚本
    """
    ...
    def __init__(self):
        ...
        self.INPUT_PATH = os.path.join(
            "LOG", "DeviceBackupTask",
            f"{TODAY_STRING}-关键设备配置备份输出EXCEL基础任务.xlsx"
        )
        ...
        self.OUTPUT_PATH = os.path.join(self.OUTPUT_DIR, f"{TODAY_STRING}-ACL无ARP匹配检查.xlsx")
    ...
    def items(self):
        ...
        try:
            ...
            sheet_names = [
                ...
                for inputWorksheet in input_workbook.worksheets
    ...
    def run(self) -> None:
        ...
        处理所有工作表，检查ACL规则在ARP表中的命中情况，
        使用xlsxwriter输出原位富文本标色，并生成操作脚本与回退脚本
        """
        # 获取所有工作表
        ...
        try:
            ...
            configuration_directory = build_log_path("ACLArpCheckTask", "ConfigureOutput")
            ensure_output_dir(configuration_directory)
            for _name in os.listdir(configuration_directory):
                _path = os.path.join(configuration_directory, _name)
            ...
            per_sheet_statistics = self._process_file_with_coloring(
                self.INPUT_PATH, output_file_path, progress
            )


            # 生成操作脚本和回退脚本（仅对有标色的工作表）
            try:
                self._generate_operation_scripts(self.INPUT_PATH, per_sheet_statistics)
            ...
            # 添加每个工作表的结果记录
            for sheet_name, statistics in per_sheet_statistics.items():
                self.add_result(
                    Level.OK,

                    f"站点{sheet_name}处理完成："
                    f"无ARP精确匹配(红色关键字标色)={statistics['red']}，"
                    f"命中NoUseIPRange模糊匹配(橙色关键字标色)={statistics['yellow']}，"
                    f"总计={statistics['total']}条规则"
                )
            ...
            if enable_summary:
                total_red = sum(
                    ...
                    for statistics in per_sheet_statistics.values()
                )
                total_yellow = sum(
                ...
                self.add_result(
                    Level.OK,
                    f"ACL无ARP匹配检查汇总：处理{len(per_sheet_statistics)}个站点，"
    ...
    def _process_file_with_coloring(
        self, input_path: str, output_file_path: str, progress=None
        ...
        acl_configuration = CONFIG["ACLArpCheckTask"]
        ...
        per_sheet_statistics: Dict[str, Dict[str, int]] = {}
        ...
        for inputWorksheet in input_workbook.worksheets:
            ...
            # 获取工作表的最大列数
            maximum_column = inputWorksheet.max_column


            # 遍历所有列，使用ACL块优化处理
            for col_idx in range(1, maximum_column + 1):
                # 找到该列中的ACL块
                ...
                for start_row, end_row in acl_blocks:
                    red_count, yellow_count, total_count, colored_rules_information = (
                        process_acl_block_with_unicode_marking(
                            inputWorksheet, worksheet, col_idx, start_row, end_row,
                            arp_ok_ip_addresses, no_use_ranges, acl_configuration,
                    ...
                    # 收集标色的规则信息
                    for rule_info in colored_rules_information:
                        sheet_colored_rules.append(rule_info)


            per_sheet_statistics[sheet_name] = {
                "red": sheet_red_count,
    ...
    def _find_acl_name_in_column(
        ...
        for ROW in range(start_row_index, max(1, start_row_index - 2000), -1):
            ...
            if cell_value and isinstance(cell_value, str):
                cell_string = str(cell_value).strip()
                # 查找ACL定义，检查是否包含extended关键字
                match = re.search(
                    r'ip access-list (?:extended )?(\S+)', cell_string, re.IGNORECASE
                )
                if match:
                    acl_name = match.group(1)
                    # 检查原始定义中是否包含extended关键字（忽略大小写）
                    has_extended = 'extended' in cell_string.lower()
    ...
    # 生成操作脚本和回退脚本（仅对有标色的工作表）
    def _generate_operation_scripts(
            self, input_path: str,
            per_sheet_statistics: Dict[str, Dict[str, int]]
    ) -> None:
        try:
            # 生成当日日期前缀，文件名保留日期
            date_string = get_today_str()
            ...
            for inputWorksheet in input_workbook.worksheets:
                ...
                # 获取统计信息
                statistics = per_sheet_statistics.get(
                    sheet_name, {"red": 0, "yellow": 0, "total": 0}
                )
                ...
                operation_file_path = os.path.join(
                    configuration_directory, f"{date_string}-{sheet_name}操作脚本.log"
                )
                rollback_file_path = os.path.join(
                    configuration_directory, f"{date_string}-{sheet_name}回退脚本.log"
                )


                with open(operation_file_path, 'w', encoding='utf-8') as operationLogFile, \
                        open(rollback_file_path, 'w', encoding='utf-8') as rollbackLogFile:
                    ...
                    operationLogFile.write(
                        f"# 统计: 红色标记={statistics['red']}, "
                        f"橙色标记={statistics['yellow']}, 总计={statistics['total']}\n\n"
                    )


                    rollbackLogFile.write(
                        f"# {sheet_name}回退脚本 - "
                        f"{format_datetime(datetime.now(), '%Y-%m-%d %H:%M:%S')}\n"
                    )
                    rollbackLogFile.write(
                        f"# 统计: 红色标记={statistics['red']}, "
                        f"橙色标记={statistics['yellow']}, 总计={statistics['total']}\n\n"
                    ...
                    for rule_info in colored_rules:
                        column_index_identifier = rule_info['column']
                        row_index_identifier = rule_info['row']


                        # 获取设备信息（从第1行获取设备名称）
                        device_name = None
                        if inputWorksheet.cell(row=1, column=column_index_identifier).value:
                            device_name = str(
                                inputWorksheet.cell(
                                    row=1, column=column_index_identifier
                                ).value
                            ).strip()


                        if device_name:
                            if column_index_identifier not in device_groups:
                                device_groups[column_index_identifier] = {
                                    'device_name': device_name,
                                    'rules': []
                                }
                            device_groups[column_index_identifier]['rules'].append(rule_info)
                    ...
                    for column_index_key, device_info in device_groups.items():
                        ...
                        for rule_info in rules:
                            row_index_identifier = rule_info['row']
                            acl_information = self._find_acl_name_in_column(
                                inputWorksheet, column_index_key,
                                row_index_identifier
                            )


                            if acl_information:
                                acl_name, has_extended = acl_information
            ...
            for sheet_name, statistics in per_sheet_statistics.items():
                if statistics["total"] > 0:
                    op_script = (
                        ...
                        f"{date_string}-{sheet_name}操作脚本.log"
                    )
                    self.add_result(Level.OK, f"生成{sheet_name}操作脚本: {op_script}")
                    rb_script = (
                        f"LOG/ACLArpCheckTask/ConfigureOutput/"
                        f"{date_string}-{sheet_name}回退脚本.log"
                    )
