...
#
# 输出文件:LOG/ACLCrossCheckTask/{日期}-跨平台N9K&LINKAS&OOB ACL交叉检查.xlsx
# 输出内容:
# - 每个Sheet保留原始配置内容
    ...
    def _step7_cross_sheet_cat1_cat2_reverse_match_check(
        ...
        if cross_match_count > 0:
            self.add_result(
                ...
                f"标记{marked_count}个单元格（跳过{skipped_count}个已标绿色的单元格）"
            )
                                ...
                                if 'permit' in cell_text.lower() or 'deny' in cell_text.lower():
                                    ...
                                    if not is_redundant:
                                        ...
                                        icmp_any_252_254_pattern = re.compile(
                                            r'\bicmp\s+any\s+(\d+\.\d+\.\d+\.(?:252|253|254))/30\b',
                                            re.IGNORECASE
                                        )
                                    ...
                                    if not is_redundant:
                                        ...
                                        if parsed_rule:
                                            # 检查源网络和目的网络是否都在平台网段内
                                            src_in_platform = self._network_in_platform(
                                                parsed_rule.src, platform_networks
                                            )
                                            dst_in_platform = self._network_in_platform(
                                            ...
                                            if networks:
                                                ...
                                                src_in_platform = self._network_in_platform(
                                                    src_network, platform_networks
                                                )
                                                dst_in_platform = self._network_in_platform(
                                                    dst_network, platform_networks
                                                )
